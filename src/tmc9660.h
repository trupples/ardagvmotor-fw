...
#if 0
/* Run DO(PARAM_NAME) macro for each TMC9660 parameter */
// #define FOREACH_TMC9660_PARAM(DO) 
#endif

/* Run DO(PARAM_NAME) macro for each RWE (non-volatile) TMC9660 parameter. Intended for saving/loading configuration parameters to/from flash. */
#define FOREACH_TMC9660_RWE_PARAM(DO) \
    DO(MOTOR_TYPE) \
    DO(MOTOR_POLE_PAIRS) \
    DO(MOTOR_DIRECTION) \
    DO(MOTOR_PWM_FREQUENCY) \
    DO(OUTPUT_VOLTAGE_LIMIT) \
    DO(MAX_TORQUE) \
    DO(MAX_FLUX) \
    DO(PWM_SWITCHING_SCHEME) \
    DO(IDLE_MOTOR_PWM_BEHAVIOR) \
    DO(ADC_SHUNT_TYPE) \
    DO(CSA_GAIN_ADC_I0_TO_ADC_I2) \
    DO(CSA_GAIN_ADC_I3) \
    DO(CSA_FILTER_ADC_I0_TO_ADC_I2) \
    DO(CSA_FILTER_ADC_I3) \
    DO(CURRENT_SCALING_FACTOR) \
    DO(PHASE_UX1_ADC_MAPPING) \
    DO(PHASE_VX2_ADC_MAPPING) \
    DO(PHASE_WY1_ADC_MAPPING) \
    DO(PHASE_Y2_ADC_MAPPING) \
    DO(ADC_I0_SCALE) \
    DO(ADC_I1_SCALE) \
    DO(ADC_I2_SCALE) \
    DO(ADC_I3_SCALE) \
    ...
    DO(OPENLOOP_CURRENT) \
    DO(OPENLOOP_VOLTAGE) \
    DO(ACCELERATION_FF_GAIN) \
    DO(ACCELERATION_FF_SHIFT) \
    DO(RAMP_ENABLE) \
    DO(DIRECT_VELOCITY_MODE) \
    DO(RAMP_AMAX) \
    DO(RAMP_A1) \
    DO(RAMP_A2) \
    DO(RAMP_DMAX) \
    ...
    DO(RAMP_TVMAX) \
    DO(RAMP_TZEROWAIT) \
    DO(ACCELERATION_FEEDFORWARD_ENABLE) \
    DO(VELOCITY_FEEDFORWARD_ENABLE) \
    DO(HALL_SECTOR_OFFSET) \
    DO(HALL_FILTER_LENGTH) \
    DO(HALL_POSITION_0_OFFSET) \
    DO(HALL_POSITION_60_OFFSET) \
    DO(HALL_POSITION_120_OFFSET) \
    DO(HALL_POSITION_180_OFFSET) \
    DO(HALL_POSITION_240_OFFSET) \
    DO(HALL_POSITION_300_OFFSET) \
    DO(HALL_INVERT_DIRECTION) \
    DO(HALL_EXTRAPOLATION_ENABLE) \
    DO(HALL_PHI_E_OFFSET) \
    DO(ABN_1_STEPS) \
    DO(ABN_1_DIRECTION) \
    DO(ABN_1_INIT_METHOD) \
    DO(ABN_1_INIT_DELAY) \
    DO(ABN_1_INIT_VELOCITY) \
    DO(ABN_1_N_CHANNEL_PHI_E_OFFSET) \
    DO(ABN_1_N_CHANNEL_INVERTED) \
    DO(ABN_1_N_CHANNEL_FILTERING) \
    DO(TORQUE_P) \
    DO(TORQUE_I) \
    DO(FLUX_P) \
    DO(FLUX_I) \
    DO(SEPARATE_TORQUE_FLUX_PI_PARAMTERS) \
    DO(CURRENT_NORM_P) \
    DO(CURRENT_NORM_I) \
    DO(VELOCITY_SENSOR_SELECTION) \
    DO(VELOCITY_P) \
    DO(VELOCITY_I) \
    DO(VELOCITY_NORM_P) \
    DO(VELOCITY_NORM_I) \
    DO(VELOCITY_SCALING_FACTOR) \
    DO(VELOCITY_LOOP_DOWNSAMPLING) \
    DO(VELOCITY_REACHED_THRESHOLD) \
    DO(VELOCITY_METER_SWITCH_THRESHOLD) \
    DO(VELOCITY_METER_SWITCH_HYSTERESIS) \
    DO(POSITION_SENSOR_SELECTION) \
    DO(POSITION_SCALING_FACTOR) \
    DO(POSITION_P) \
    DO(POSITION_I) \
    DO(POSITION_NORM_P) \
    DO(POSITION_NORM_I) \
    DO(STOP_ON_POSITION_DEVIATION) \
    DO(POSITION_LOOP_DOWNSAMPLING) \
    DO(POSITION_LIMIT_LOW) \
    DO(POSITION_LIMIT_HIGH) \
    DO(POSITION_REACHED_THRESHOLD) \
    DO(REFERENCE_SWITCH_ENABLE) \
    DO(REFERENCE_SWITCH_POLARITY_AND_SWAP) \
    DO(REFERENCE_SWITCH_LATCH_SETTINGS) \
    DO(EVENT_STOP_SETTINGS) \
    DO(REFERENCE_SWITCH_SEARCH_MODE) \
    DO(REFERENCE_SWITCH_SEARCH_SPEED) \
    DO(REFERENCE_SWITCH_SPEED) \
    DO(ABN_2_STEPS) \
    DO(ABN_2_DIRECTION) \
    DO(ABN_2_GEAR_RATIO) \
    DO(ABN_2_ENABLE) \
    DO(SPI_ENCODE_CS_SETTLE_DELAY_TIME) \
    DO(SPI_ENCODER_CS_IDLE_DELAY_TIME) \
    DO(SPI_ENCODER_MAIN_TRANSFER_CMD_SIZE) \
    DO(SPI_ENCODER_SECONDARY_TRANSFER_CMD_SIZE) \
    DO(SPI_ENCODER_TRANSFER_DATA_3_0) \
    DO(SPI_ENCODER_TRANSFER_DATA_7_4) \
    DO(SPI_ENCODER_TRANSFER_DATA_11_8) \
    DO(SPI_ENCODER_TRANSFER_DATA_15_12) \
    DO(SPI_ENCODER_TRANSFER) \
    DO(SPI_ENCODER_POSITION_COUNTER_MASK) \
    DO(SPI_ENCODER_POSITION_COUNTER_SHIFT) \
    DO(SPI_ENCODER_INITIALIZATION_METHOD) \
    DO(SPI_ENCODER_DIRECTION) \
    DO(SPI_ENCODER_OFFSET) \
    DO(SPI_LUT_CORRECTION_ENABLE) \
    DO(STEP_DIR_STEP_DIVIDER_SHIFT) \
    DO(BRAKE_CHOPPER_ENABLE) \
    DO(BRAKE_CHOPPER_VOLTAGE_LIMIT) \
    DO(BRAKE_CHOPPER_HYSTERESIS) \
    DO(RELEASE_BRAKE) \
    DO(BRAKE_RELEASING_DUTY_CYCLE) \
    DO(BRAKE_HOLDING_DUTY_CYCLE) \
    DO(BRAKE_RELEASING_DURATION) \
    DO(INVERT_BRAKE_OUTPUT) \
    DO(THERMAL_WINDING_TIME_CONSTANT_1) \
    DO(IIT_LIMIT_1) \
    DO(THERMAL_WINDING_TIME_CONSTANT_2) \
    DO(IIT_LIMIT_2) \
    DO(PWM_L_OUTPUT_POLARITY) \
    DO(PWM_H_OUTPUT_POLARITY) \
    DO(BREAK_BEFORE_MAKE_TIME_LOW_UVW) \
    DO(BREAK_BEFORE_MAKE_TIME_HIGH_UVW) \
    DO(BREAK_BEFORE_MAKE_TIME_LOW_Y2) \
    DO(BREAK_BEFORE_MAKE_TIME_HIGH_Y2) \
    DO(USE_ADAPTIVE_DRIVE_TIME_UVW) \
    DO(USE_ADAPTIVE_DRIVE_TIME_Y2) \
    DO(DRIVE_TIME_SINK_UVW) \
    DO(DRIVE_TIME_SOURCE_UVW) \
    DO(DRIVE_TIME_SINK_Y2) \
    DO(DRIVE_TIME_SOURCE_Y2) \
    DO(UVW_SINK_CURRENT) \
    DO(UVW_SOURCE_CURRENT) \
    DO(Y2_SINK_CURRENT) \
    DO(Y2_SOURCE_CURRENT) \
    DO(BOOTSTRAP_CURRENT_LIMIT) \
    DO(UNDERVOLTAGE_PROTECTION_SUPPLY_LEVEL) \
    DO(UNDERVOLTAGE_PROTECTION_VDRV_ENABLE) \
    DO(UNDERVOLTAGE_PROTECTION_BST_UVW_ENABLE) \
    DO(UNDERVOLTAGE_PROTECTION_BST_Y2_ENABLE) \
    DO(OVERCURRENT_PROTECTION_UVW_LOW_SIDE_ENABLE) \
    DO(OVERCURRENT_PROTECTION_UVW_HIGH_SIDE_ENABLE) \
    DO(OVERCURRENT_PROTECTION_Y2_LOW_SIDE_ENABLE) \
    DO(OVERCURRENT_PROTECTION_Y2_HIGH_SIDE_ENABLE) \
    DO(OVERCURRENT_PROTECTION_UVW_LOW_SIDE_THRESHOLD) \
    DO(OVERCURRENT_PROTECTION_UVW_HIGH_SIDE_THRESHOLD) \
    DO(OVERCURRENT_PROTECTION_Y2_LOW_SIDE_THRESHOLD) \
    DO(OVERCURRENT_PROTECTION_Y2_HIGH_SIDE_THRESHOLD) \
    DO(OVERCURRENT_PROTECTION_UVW_LOW_SIDE_BLANKING) \
    DO(OVERCURRENT_PROTECTION_UVW_HIGH_SIDE_BLANKING) \
    DO(OVERCURRENT_PROTECTION_Y2_LOW_SIDE_BLANKING) \
    DO(OVERCURRENT_PROTECTION_Y2_HIGH_SIDE_BLANKING) \
    DO(OVERCURRENT_PROTECTION_UVW_LOW_SIDE_DEGLITCH) \
    DO(OVERCURRENT_PROTECTION_UVW_HIGH_SIDE_DEGLITCH) \
    DO(OVERCURRENT_PROTECTION_Y2_LOW_SIDE_DEGLITCH) \
    DO(OVERCURRENT_PROTECTION_Y2_HIGH_SIDE_DEGLITCH) \
    DO(OVERCURRENT_PROTECTION_UVW_LOW_SIDE_USE_VDS) \
    DO(OVERCURRENT_PROTECTION_Y2_LOW_SIDE_USE_VDS) \
    DO(VGS_SHORT_ON_PROTECTION_UVW_LOW_SIDE_ENABLE) \
    DO(VGS_SHORT_OFF_PROTECTION_UVW_LOW_SIDE_ENABLE) \
    DO(VGS_SHORT_ON_PROTECTION_UVW_HIGH_SIDE_ENABLE) \
    DO(VGS_SHORT_OFF_PROTECTION_UVW_HIGH_SIDE_ENABLE) \
    DO(VGS_SHORT_ON_PROTECTION_Y2_LOW_SIDE_ENABLE) \
    DO(VGS_SHORT_OFF_PROTECTION_Y2_LOW_SIDE_ENABLE) \
    DO(VGS_SHORT_ON_PROTECTION_Y2_HIGH_SIDE_ENABLE) \
    DO(VGS_SHORT_OFF_PROTECTION_Y2_HIGH_SIDE_ENABLE) \
    DO(VGS_SHORT_PROTECTION_UVW_BLANKING) \
    DO(VGS_SHORT_PROTECTION_Y2_BLANKING) \
    DO(VGS_SHORT_PROTECTION_UVW_DEGLITCH) \
    DO(VGS_SHORT_PROTECTION_Y2_DEGLITCH) \
    DO(GDRV_RETRY_BEHAVIOUR) \
    DO(DRIVE_FAULT_BEHAVIOUR) \
    DO(FAULT_HANDLER_NUMBER_OF_RETRIES) \
    DO(SUPPLY_OVERVOLTAGE_WARNING_THRESHOLD) \
    DO(SUPPLY_UNDERVOLTAGE_WARNING_THRESHOLD) \
    DO(EXTERNAL_TEMPERATURE_SHUTDOWN_THRESHOLD) \
    DO(EXTERNAL_TEMPERATURE_WARNING_THRESHOLD) \
    DO(CHIP_TEMPERATURE_SHUTDOWN_THRESHOLD) \
    DO(CHIP_TEMPERATURE_WARNING_THRESHOLD) \
    DO(FIELDWEAKENING_I) \
    DO(FIELDWEAKENING_VOLTAGE_THRESHOLD) \
    DO(TARGET_TORQUE_BIQUAD_FILTER_ENABLE) \
    DO(TARGET_TORQUE_BIQUAD_FILTER_ACOEFF_1) \
    DO(TARGET_TORQUE_BIQUAD_FILTER_ACOEFF_2) \
    DO(TARGET_TORQUE_BIQUAD_FILTER_BCOEFF_0) \
    DO(TARGET_TORQUE_BIQUAD_FILTER_BCOEFF_1) \
    DO(TARGET_TORQUE_BIQUAD_FILTER_BCOEFF_2) \
    DO(ACTUAL_VELOCITY_BIQUAD_FILTER_ENABLE) \
    DO(ACTUAL_VELOCITY_BIQUAD_FILTER_ACOEFF_1) \
    DO(ACTUAL_VELOCITY_BIQUAD_FILTER_ACOEFF_2) \
    DO(ACTUAL_VELOCITY_BIQUAD_FILTER_BCOEFF_0) \
    DO(ACTUAL_VELOCITY_BIQUAD_FILTER_BCOEFF_1) \
    DO(ACTUAL_VELOCITY_BIQUAD_FILTER_BCOEFF_2)
